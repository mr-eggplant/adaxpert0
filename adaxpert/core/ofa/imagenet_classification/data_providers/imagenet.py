...
from core.ofa.utils.my_dataloader import MyRandomResizedCrop, MyDistributedSampler
...
class ImagenetDataProvider(DataProvider):
	...
	def __init__(self, save_path=None, train_batch_size=256, test_batch_size=512, valid_size=None, n_worker=32,
	             ...
	             num_replicas=None, rank=None, test_split_size=0.01):
		...
		if test_split_size is not None:
			if not isinstance(test_split_size, int):
				assert isinstance(test_split_size, float) and 0 < test_split_size < 1
				test_split_size = int(len(test_dataset) * test_split_size)

			
			# valid_dataset = self.train_dataset(valid_transforms)
			_, partial_indexes = self.random_sample_valid_set(len(test_dataset), test_split_size)

			if num_replicas is not None:
				partial_test_sampler = MyDistributedSampler(train_dataset, num_replicas, rank, True, np.array(partial_indexes))
			else:
				partial_test_sampler = torch.utils.data.sampler.SubsetRandomSampler(partial_indexes)

			self.test = torch.utils.data.DataLoader(
				test_dataset, batch_size=test_batch_size, sampler=partial_test_sampler,
				num_workers=n_worker, pin_memory=True,
			)
		else:
			if num_replicas is not None:
