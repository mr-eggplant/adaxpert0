...
from core.ofa.utils import get_net_info, cross_entropy_loss_with_soft_target, cross_entropy_with_label_smoothing
from core.ofa.utils import AverageMeter, accuracy, write_log, mix_images, mix_labels, init_models
from core.ofa.utils import MyRandomResizedCrop
...
class RunManager:
    ...
    def reset_running_statistics(self, net=None, subset_size=2000, subset_batch_size=200, data_loader=None):
        from core.ofa.imagenet_classification.elastic_nn.utils import set_running_statistics
        if net is None:
            net = self.network
        if data_loader is None:
